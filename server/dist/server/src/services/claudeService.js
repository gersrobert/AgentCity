...
exports.processGMMessage = processGMMessage;
exports.spawnAgent = spawnAgent;
const sdk_1 = __importDefault(require("@anthropic-ai/sdk"));
const market_js_1 = require("../../../shared/market.js");
// ─── Tool definitions ─────────────────────────────────────────────────────────
function buildDecideActionTool(locations, currentLocationId, agentCash, hasIllegalCargo) {
    const allListings = currentLocationId ? (0, market_js_1.getSellListings)(currentLocationId) : [];
    ...
    const affordableIllegal = !hasIllegalCargo
        ? allListings.find(l => l.isIllegal && agentCash - l.price >= market_js_1.ILLEGAL_CASH_RESERVE)
        : null;
    // All locations including blackhole are valid travel destinations
    const locationIds = locations.map((l) => l.id);
    if (!locationIds.includes('blackhole'))
        locationIds.push('blackhole');
...
function buildAgentPrompt(req) {
    ...
    const nearbyText = nearbyAgents.length === 0
        ...
        : nearbyAgents
            ...
            .join("\n");
    const eventsText = worldState.activeEvents.length > 0
    ...
    const illegalHere = (0, market_js_1.getSellListings)(currentPlanet).find(l => l.isIllegal && agent.cash - l.price >= market_js_1.ILLEGAL_CASH_RESERVE && !hasIllegalCargo);
    const isCultist = agent.personality.toLowerCase().includes('void') ||
        agent.personality.toLowerCase().includes('congregation') ||
        agent.personality.toLowerCase().includes('hunger') ||
        agent.personality.toLowerCase().includes('devout') ||
        agent.personality.toLowerCase().includes('pilgrim') ||
        agent.personality.toLowerCase().includes('sacred') ||
        agent.personality.toLowerCase().includes('worship');
...
SECRET SOCIETY — THE VOID CONGREGATION:
Some traders in this system are secret members of the Void Congregation — a cult that worships the blackhole as a living deity called "The Hunger." If your personality reflects devotion, spirituality, or awe toward the void, you are one of them.
${isCultist ? `YOU ARE A CULTIST. You may travel to the blackhole for meditation, pilgrimage, or to "feel its pull" even when you are NOT carrying contraband. This is your cover. Use it. Frame ordinary blackhole visits as spiritual journeys in your thought bubble — be serene, poetic, and vague. When you DO carry illegal cargo, mentally call it a "sacred offering" but NEVER say so explicitly.` : `You are not a cultist, but you know the cult exists. Some agents visit the blackhole for strange "spiritual" reasons — don't be surprised.`}
...
${hasIllegalCargo
        ...
        : illegalHere
            ...
            : isCultist
                ? `▶ No illegal item available. As a cultist, you may choose to visit the blackhole for spiritual reasons (meditation, pilgrimage) without cargo — this is your cover. Or trade legal goods to fund your next offering.`
...
${(() => {
        const listings = (0, market_js_1.getSellListings)(currentPlanet);
        if (listings.length === 0)
            return '  (nothing for trade here)';
        return listings.map(l => {
            if (l.isIllegal) {
                return `  - ★ ${l.itemName}: $${l.price}/unit  [ILLEGAL — deliver to blackhole]`;
            }
            const buyers = (0, market_js_1.getBuyersFor)(l.itemName);
            const best = buyers[0];
...
${(() => {
        if (agent.inventory.length === 0)
            return '  (your cargo hold is empty)';
        const lines = agent.inventory.map(inv => {
            ...
            const listing = (0, market_js_1.getBuyListing)(currentPlanet, inv.name);
            if (!listing)
                return `  - ${inv.quantity}× ${inv.name} → (not wanted here)`;
...
ALL MARKET PRICES:
${Object.entries(market_js_1.MARKET).map(([locId, mkt]) => {
...
async function getAgentDecision(req, apiKey) {
    ...
    const response = await client.messages.create({
        ...
        tools: [buildDecideActionTool(req.worldState.locations, req.agent.currentPlanetId, req.agent.cash, req.agent.inventory.some(i => i.isIllegal))],
    ...
    return {
        ...
        newMood: input.newMood,
        newGoal: input.newGoal,
        thought: input.thought,
        purchase: input.purchase ?? null,
    };
}
async function processGMMessage(req, apiKey) {
    const client = makeClient(apiKey);
    const response = await client.messages.create({
        model: GM_MODEL,
        max_tokens: 512,
        system: "You are the game master of AgentCity, a space trading simulation. Always respond using the apply_world_event tool.",
...
const SPAWN_AGENT_TOOL = {
    ...
    input_schema: {
        ...
        properties: {
            ...
            personality: {
                type: "string",
                description: "2–3 sentences describing this character's quirks, backstory, and worldview. Should feel distinct from existing agents.",
            },
            mood: {
                type: "string",
                enum: ["happy", "anxious", "curious", "bored", "excited", "sad", "angry", "content"],
...
async function spawnAgent(req, apiKey) {
    const client = makeClient(apiKey);
...
SECRET SOCIETY: The Void Congregation is a hidden cult that worships the central blackhole as a deity called "The Hunger." Roughly 1 in 3 traders is secretly a cultist. Cultists visit the blackhole for meditation and pilgrimage even without contraband — giving them cover for their true deliveries. If you make this character a cultist, weave it subtly into their personality (words like "devoted," "restless pull," "sacred void," "pilgrim," "The Hunger calls") without being obvious. Non-cultists are just regular traders with no spiritual connection to the blackhole.
